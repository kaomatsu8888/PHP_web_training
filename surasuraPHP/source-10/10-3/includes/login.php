<?php
  session_start();  // セッション開始。これがないと、このページに来たときにセッションが開始されない＝ログイン状態が保持されない

  if (isset($_SESSION['id'])){// もしセッションにidがあれば、つまりログインしている状態なら
    // セッションにユーザIDがある=ログインしている
    // ログイン済みならトップページに遷移する
    header('Location: index.php');// ログインしている場合はindex.phpに遷移
    ...
    try {
      ...
      // パラメータ割り当て
      $stmt->bindParam(':name', $_POST['name'], PDO::PARAM_STR);// PARAM_STRは文字列型
