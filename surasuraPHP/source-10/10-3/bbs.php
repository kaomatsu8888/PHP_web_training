        ...
        <div class="card">
          ...
          <div class="card-footer">
            ...
            <form action="delete.php" method="post" class="form-inline">
              <?php echo $row['name'] ?>
              (<?php echo $row['date'] ?>)
              <!-- hiddenとは、画面には表示されないが、フォームのデータとして送信されるデータ -->
              <!-- postとの違いは、URLに表示されないこと -->
              <!-- hiddenにする理由は、削除するためのIDを送信するため -->
      ...
      <?php while ($row = $stmt->fetch()): ?>
        <div class="card">
          ...
          <div class="card-body">
            <!-- <p class="card-text"><?php //echo nl2br($row['body']) ?></p> -->
            <!-- エスケープを反映 -->
            <p class="card-text"><?php echo htmlspecialchars($row['body'], ENT_QUOTES, 'UTF-8') ?></p>
          </div>
          <div class="card-footer">
